...
public enum FilmRequests {
    ...
    INSERT_QUERY("INSERT INTO films (name, description, release_date, duration, likes_count, mpa_id) " +
                 "VALUES (?, ?, ?, ?, ?, ?)"),
    UPDATE_QUERY("UPDATE films SET name = ?, description = ?, release_date = ?, duration = ?, likes_count = ?, mpa_id = ? " +
                 "WHERE id = ?"),
    ...
    POPULAR_FILM_QUERY("SELECT * FROM films f JOIN (SELECT film_id, COUNT(user_id) AS like_count" +
                       " FROM likes GROUP BY film_id ORDER BY like_count DESC) l ON f.id=l.film_id LIMIT ?");

    public final String query;

    FilmRequests(String query) {
        this.query = query;
    }
}
